...
locals {
  ...
  alert_documentation = <<-EOT
    GTM Monitoring issues detected in project: ${var.project_id}.
    This alert fires when a Dataform assertion fails.
    Check the extracted labels and the logs for further details!

    This monitoring message is generated by the Data to Value GTM Monitor: https://github.com/datatovalue/gtm-monitoring-gcp.
  EOT
}
